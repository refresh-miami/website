	...
	<div class="header-aside-inner">
		<?php if ( is_user_logged_in() ) : ?>
			<div class="user-wrap user-wrap-container menu-item-has-children">
                ...
                $xprofile_nick_name  = xprofile_get_field_data( $nickname_id, $current_user->ID );
                $xprofile_first_name = xprofile_get_field_data( $first_name_id, $current_user->ID );
                $xprofile_last_name  = xprofile_get_field_data( $last_name_id, $current_user->ID );

                if ( '' === $xprofile_first_name ) {
                    $result_first_name = get_user_meta( $current_user->ID, 'first_name', true );
                    if ( empty( $result_first_name ) ) {
                        $result_first_name = get_user_meta( $current_user->ID, 'nickname', true );
                    }
                } else {
                    $result_first_name = $xprofile_first_name;
                }

				?>

				<a class="user-link" href="<?php echo $user_link; ?>">
					<span class="user-name"><?php echo $result_first_name; ?></span><i class="bb-icon-angle-down"></i>
		...
		<?php else : ?>
			...
			<div class="bb-header-buttons">
				...
				<?php if ( get_option( 'users_can_register' ) ) : ?>
					<a href="<?php echo '/pricing'; //wp_registration_url(); ?>" class="button small singup"><?php _e( 'Sign up', 'buddyboss-theme' ); ?></a>
				<?php endif; ?>
			</div>
